...
class MocapPX4Bridge : public rclcpp::Node {
  public:
    MocapPX4Bridge() : Node("mocap_px4_bridge") {
        this->declare_parameter("mocap_topic", "/vrpn_mocap/drone1/pose");
        this->declare_parameter("px4_topic", "/fmu/in/vehicle_visual_odometry");

        const std::string mocap_topic =
            this->get_parameter("mocap_topic").as_string();
        ...
        poseSub = this->create_subscription<geometry_msgs::msg::PoseStamped>(
            mocap_topic, 10,
            std::bind(&MocapPX4Bridge::poseCallback, this, _1));
        odomPub = this->create_publisher<px4_msgs::msg::VehicleOdometry>(
            px4_topic, 10);
    }
...
void MocapPX4Bridge::poseCallback(
    const geometry_msgs::msg::PoseStamped::UniquePtr poseMsg) {
    ...
    RCLCPP_INFO_ONCE(get_logger(), "q: %f, %f, %f, %f",
                     poseMsg->pose.orientation.w, poseMsg->pose.orientation.x,
    ...
    odomMsg.timestamp = uint64_t(poseMsg->header.stamp.sec) * 1000000 +
                        uint64_t(poseMsg->header.stamp.nanosec) / 1000;
